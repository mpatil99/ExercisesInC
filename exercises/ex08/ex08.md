...
GLib is from 5 low level libraries. GLib provides advanced Data Structures and Threads among other features. Part of the GNOME Project which is part of the GNU Project. GLib was initially part of the GTK+ project however, was seperated from the GUI code before GTK+ version 2. It may have been better as not to confuse people with the glibc which is the GNU Project implementation of the C standard library.

*  What are the most important pros and cons of using GLib, as opposed
to another library, or just using core C features?
One of the most evident pros is that that GLib provides a set of well tested data structures.
A con is that including GLib will add a substantial number of lines of code to your executable files.

*  What do you have to do to compile and run a "Hello GLib" example?
You would need to create a new GSting using g_string_new, then use g_print. Finally, you would need to use g_string_free.

In order to compile you would need compile you would need
